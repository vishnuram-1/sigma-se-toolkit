...
if [ -z "${SIGMA_API_TOKEN:-}" ]; then
  ...
  if $_fresh; then
    ...
    # Resolve a token fetcher, most specific first, so this works in a fresh
    # clone without configuration. The in-repo fork is preferred: it extends
    # the host allowlist beyond what upstream's copy accepts.
    _gettoken="${SIGMA_TOKEN_FETCHER:-}"
    if [ -z "$_gettoken" ]; then
      for _candidate in \
        "${_sigma_repo_root}/scripts/api/get-token.sh" \
        "${_sigma_repo_root}/.claude/skills/sigma-api/scripts/get-token.sh" \
        "$HOME/.claude/skills/sigma-api/scripts/get-token.sh" \
        "$HOME/.claude/skill-sources/sigma-agent-skills/skills/sigma-api/scripts/get-token.sh"
      do
        if [ -f "$_candidate" ]; then _gettoken="$_candidate"; break; fi
      done
    fi
    if [ -z "$_gettoken" ] || [ ! -f "$_gettoken" ]; then
      echo "_env.sh: no get-token.sh found. Looked for:" >&2
      echo "  \$SIGMA_TOKEN_FETCHER" >&2
      echo "  ${_sigma_repo_root}/scripts/api/get-token.sh" >&2
      echo "  ${_sigma_repo_root}/.claude/skills/sigma-api/scripts/get-token.sh" >&2
      echo "Set SIGMA_TOKEN_FETCHER to override." >&2
...
sigma_curl() {
  ...
  while :; do
    ...
    if [ "$_status" = "401" ] && [ "$_retries" -eq 0 ]; then
      ...
      # Re-source this same file to mint a fresh token, then retry once.
